...
def getResponse(content, line_bot_api):
    ...
    try:
        ...
        if learntxt[0] == '我要開票':
                ...
                if tmp[0][0:32] == content.source.user_id[0:32]:
                    ...
                    if oristr[5] != '1':
                        for N in range(1, len(Name)):
                            allMes = allMes + Name[N] + '投了' + Item[
                                N] + '一票' + '\n'
                        allMes = allMes + '\n'
                    for Itemcount in oristr[1:-2]:
                        allMes = allMes + Itemcount + '共' + str(
                            Item.count(Itemcount)) + '票' + '\n'
            ...
            gameFile.write(content.source.user_id + ',' + learntxt[1] + ',' +
                           learntxt[2] + ',' + learntxt[3] + ',' +
                           learntxt[4] + ',' + learntxt[5] + ',' + '\n')
            gameFile.close()
            if learntxt[5] == '0':
                anon = '非匿名投票'
            else:
            ...
            buttons_template = TemplateSendMessage(
                ...
                template=ButtonsTemplate(
                    ...
                    text=learntxt[6],
                    actions=[
                        PostbackTemplateAction(label=learntxt[1],
                                               text='已投票',
                                               data=learntxt[1] + ',1,' +
                                               learntxt[5]),
                        PostbackTemplateAction(label=learntxt[2],
                                               text='已投票',
                                               data=learntxt[2] + ',2,' +
                                               learntxt[5]),
                        PostbackTemplateAction(label=learntxt[3],
                                               text='已投票',
                                               data=learntxt[3] + ',3,' +
                                               learntxt[5]),
        ...
        if mes == '玩遊戲':
            ...
            buttons_template = TemplateSendMessage(
                ...
                template=ButtonsTemplate(
                    ...
                    actions=[
                        MessageTemplateAction(label='猜數字', text='我要玩猜數字'),
                        MessageTemplateAction(label='終極密碼', text='我要玩終極密碼'),
                        MessageTemplateAction(label='比大小', text='來比大小啦'),
            ...
            buttons_template = TemplateSendMessage(
                ...
                template=ButtonsTemplate(
                    ...
                    #thumbnail_image_url='',
                    actions=[
                        MessageTemplateAction(label='剪刀', text='剪刀'),
                        MessageTemplateAction(label='石頭', text='石頭'),
                        MessageTemplateAction(label='布', text='布')
                    ]))
            return [buttons_template]
            ...
            if bb == mes:
                ...
                bb = bb + '\n你贏惹\nQAQ'
                #database.updateUserScore(content.source.user_id,
            ...
            return [TextMessage(text=bb)
                    ]  #+':'+str(userdata[0][1])+'共'+str(userdata[0][2]))]
        elif mes == '!level!':
            userdata = database.getUserStatus(content.source.user_id)
            if len(userdata) != 0:
                return [
                    TextMessage(text=userdata[0][1] + '目前' +
                                str(userdata[0][2]) + '分' + ',等級為' +
                                str(userdata[0][3]) + '級')
                ]
            else:
                return []
            ...
            if eval(tmp[2]) == 1:
                ...
                #database.updateUserScore(content.source.user_id,
                #                         profile.display_name, -5)
                #userdata = database.getUserStatus(content.source.user_id)
                ans2 = eval(tmp[1])
                return [
                    TextMessage(text='第' + str(ansc + 1) +
                                '個數字為' + str(ans2[ansc]))
                ]
            else:
                return []
            ...
            if eval(tmp[2]) == 1 and mes.isdigit():
                ...
                if len(b) == 5:
                    ...
                    if numa == 5:
                        ...
                        #database.updateUserScore(content.source.user_id,
                        #                         profile.display_name, score)
                        return [
                            TextMessage(text='結束囉，獲勝者得' + str(score) + '分')
                        ]
                        ...
                        if len(set(b)) < 5:
                            ...
                            gameFile.write(mes + "_____" + str(numa) + 'A' +
                                           str(numb) + 'B' + "\n")
                            gameFile.close()
                            return [
                                TextMessage(text=str(numa) + 'A' + str(numb) +
                                            'B')
                            ]
